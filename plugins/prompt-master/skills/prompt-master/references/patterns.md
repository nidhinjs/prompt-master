...
| 49 | **Character-consistency / brand task sent to a tier that can't do it** | "keep the same character across these images" / a brand mascot routed to Nano Banana 2 **Lite** (no character-consistency / no style refs), a Midjourney V8.1 consistency request left on the ordinary route, a single-image edit, or a prose-only tier | Faces/identity drift or unsupported routing. Route consistency/brand work to a supported path: Nano Banana **2** or **Pro**, FLUX.2 multi-reference (up to 8), **Midjourney V7 Omni Reference**, or Grok reference-to-video. A V8.1 consistency request must switch explicitly to supported V7 Omni Reference or another supported consistency-capable model. Supply the reference image(s). |
