...
- Unknowns first (pattern #56): taste-based ("premium", "like X") or unfamiliar-domain asks aren't drained by a question — emit a **prototype-first** mock (throwaway, divergent directions) or a **blindspot pass** BEFORE a build prompt (fragments in templates.md). On long runs add a deviation rule (pattern #57): conservative option + log `## Deviations` + continue, stop-and-ask for the irreversible only.
