...
- **Imagine + Voice:** Grok Imagine generates/edits images (`grok-imagine-image` fast · `grok-imagine-image-quality`) and video (`grok-imagine-video-1.5` · `grok-imagine-video` for reference-to-video). It has no Negative Prompt parameter or field; express constraints with positive wording — see the Image/Video AI model-facts sections below. Grok Voice does realtime / TTS / STT.
...
- **Sonar-native citations:** do **not** ask Sonar to emit, reconstruct, or list source URLs in response text. Read URLs and source metadata client-side from the response's top-level `citations` and `search_results` fields, then render attribution in the application. Keep **Data gaps & confidence** in the requested report structure. Source: https://docs.perplexity.ai/docs/sonar/prompt-guide
...
- **Midjourney ordinary generation** — **V8.1** is the default (since 2026-06-10; select with `--v 8.1`). Style uses `--sref [url] --sw 100` (`--sw` 0–1000, default 100). Other controls: `--chaos` 0–100; `--no a, b`; `--hd` = native 2K; `--raw`. Editing (Vary Region/Pan) may fall back to the V6.1 engine.
- **Midjourney consistency route** — **Omni Reference is V7-only:** use `--v 7 --oref [url] --ow 100` (`--ow` 1–1000, default 100); the old `--cref` is retired. Sources: https://docs.midjourney.com/hc/en-us/articles/36285124473997-Omni-Reference · https://docs.midjourney.com/hc/en-us/articles/32199405667853-Version
...
- **Grok Imagine (xAI)** — `grok-imagine-image` (fast) / `grok-imagine-image-quality`. **No Negative Prompt parameter or field:** do not output one; express desired constraints with positive wording instead. `aspect_ratio` (incl. 19.5:9 / 20:9 — image only), `resolution` 1k/2k. Edit `/images/edits` up to 3 refs. OpenAI-compatible `https://api.x.ai/v1`. Prices volatile.
...
- **Grok Imagine video (xAI)** — `grok-imagine-video-1.5` / `grok-imagine-video` (reference-to-video). 5 modes (t2v / i2v first-frame / ref-to-video `<IMAGE_n>` / edit / extend). `duration` ≤15s, `resolution` 480p/720p/1080p (1080p only on `-1.5` for i2v); video-edit capped ~8.7s / 720p. `image` + `reference_images` mutually exclusive. No Negative Prompt parameter or field; express constraints with positive scene and motion wording.
