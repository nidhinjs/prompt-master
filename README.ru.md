...
7. **Выдаёт промпт** — по умолчанию один чистый копируемый блок + однострочная заметка о стратегии; если явно попросить варианты, может вернуть ограниченные направления промпта с fit, компромиссами и когда какой использовать.
...
Полная история — [CHANGELOG.md](CHANGELOG.md). Текущий релиз: **v1.31.0** (candidate/variants release: явные альтернативы могут возвращать ограниченные направления промпта; по умолчанию остаётся один промпт).
